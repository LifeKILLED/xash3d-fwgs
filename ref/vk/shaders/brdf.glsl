...
float ggxSmithG1(float alpha, float n_dot_v) {
	n_dot_v = max(n_dot_v, 1e-6);
	const float a2 = alpha * alpha;
	return 2.0 * n_dot_v / (n_dot_v + sqrt(a2 + (1.0 - a2) * n_dot_v * n_dot_v));
}

// PDF of the reflection direction generated by SampleGGXReflection().
// The sampler currently receives material.roughness directly as GGX alpha, so the PDF uses the same convention.
float ggxReflectionPdf(vec3 N, vec3 V, vec3 L, float roughness) {
	...
	const float h_dot_n = max(dot(H, N), 0.0);
	const float h_dot_v = max(dot(H, V), 0.0);
	if (h_dot_n <= 1e-6 || h_dot_v <= 1e-6) {
		return 0.0;
	}

	const float alpha = max(roughness, 1e-4);
	const float a2 = alpha * alpha;
	const float D = ggxD(a2, h_dot_n);
	const float G1 = ggxSmithG1(alpha, n_dot_v);
	return D * G1 / max(4.0 * n_dot_v, 1e-6);
}
