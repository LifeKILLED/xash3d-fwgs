...
layout(set = 0, binding = 4,  rgba16f) uniform writeonly image2D out_radiance_temp;

layout(set = 0, binding = 5) uniform UBO { UniformBuffer ubo; } ubo;

// Utility functions (copy here)
...
void main() {
    ...
    vec3 c0  = imageLoad(INPUT_RADIANCE, pix).xyz;
    vec3 cx  = imageLoad(INPUT_RADIANCE, clampedPix(pix + ivec2(1,0), res)).xyz;
    vec3 cy  = imageLoad(INPUT_RADIANCE, clampedPix(pix + ivec2(0,1), res)).xyz;
    ...
    #ifdef SPECULAR_FIX
        float tight = mix(0.25, 1.0, R0); // low roughness → tighter
    #else
        float tight = 1.0;
    #endif

        vec3 diff = imageLoad(INPUT_RADIANCE, clampedPix(c, res)).rgb;
        diffAcc += diff * w * tight;
    ...
    imageStore(out_radiance_temp, pix, vec4(diffAcc,1));
}
