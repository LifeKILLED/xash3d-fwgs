...
#define USE_CLUSTERS
	...
	if (num_point_lights == 0) return;
	for (uint j = 0; j < num_point_lights; ++j) {
		const uint i = uint(light_grid.clusters_[cluster_index].point_lights[j]);
#else
	if (lights.m.num_point_lights == 0) return;
	for (uint i = 0; i < lights.m.num_point_lights; ++i) {
#endif
		sampleSinglePointLight(P, N, throughput, view_dir, material, lights.m.point_lights[i], true, diffuse, specular);
	} // for all lights
}
...
void computePointLightsImportance(vec3 P, vec3 N, uint cluster_index, vec3 throughput, vec3 view_dir, MaterialProperties material, out vec3 diffuse, out vec3 specular) {
	...
	for (uint i = 0; i < lights.m.num_point_lights; ++i) {
		...
		sampleSinglePointLight(P, N, throughput, view_dir, material, lights.m.point_lights[i], false, curr_diffuse, curr_specular);

		light_weights_sum += max(0., luminance(curr_diffuse + curr_specular));
	} // for all lights
	...
	if (light_weights_sum > 0.)	{
		...
		for (uint i = 0; i < lights.m.num_point_lights; ++i) {
			...
			// sample sky separately without weights
			if (lights.m.point_lights[i].environment != 0) {
				continue;
			}
			...
			const float curr_weight = max(0., luminance(curr_diffuse + curr_specular));
			sample_rnd_start = sample_rnd_end;
			sample_rnd_end += curr_weight;

			// russian roulette by light weights
			bool sample_chosen = false;
			for (uint a = 0; a < ADAPT_LIGHTS_COUNT; a++) {
				if (sample_random_pos[a] > sample_rnd_start && sample_random_pos[a] < sample_rnd_end) {
					sampleSinglePointLight(P, N, throughput, view_dir, material, lights.m.point_lights[i], true, curr_diffuse, curr_specular);
					diffuse += curr_diffuse * curr_weight / light_weights_sum;
					specular += curr_specular * curr_weight / light_weights_sum;
				}
			}
		} // for all lights

		// sample sky without random
		if (sun_light_index != -1) {
			vec3 curr_diffuse = vec3(0.);
			vec3 curr_specular = vec3(0.);
			sampleSinglePointLight(P, N, throughput, view_dir, material, lights.m.point_lights[sun_light_index], true, curr_diffuse, curr_specular);
