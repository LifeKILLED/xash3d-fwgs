...
float polyWeight(const PolygonLight poly, const vec3 P, const vec3 N) {
	...
	const float weight = dist * shading * coplanar_pow * luminanceM(poly.emissive);
...
void main() {
...
#else
	{
		...
		for (uint i = start; i < end; i++) {
			...
			const float curr_weight = luminanceM(polyShadingApproximate(poly, P, LIGHT_WEIGHT_NORMAL));
