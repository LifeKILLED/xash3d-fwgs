...
// for packing float to int
#define WEIGHTS_STORE_MULTIPLIER 5000.
...
layout(set = 0, binding = 16, rgba16i) uniform readonly iimage2D SRC_LIGHTS_CHOSEN;
#endif
	...
	layout(set=0, binding=21, rgba16f) uniform writeonly image2D OUT_SPECULAR;
		...
		layout(set=0, binding=20, rgba16i) uniform writeonly iimage2D OUT_LIGHTING;
	#else
...
void main() {
	...
	#else
		#if LIGHT_CHOOSE_PASS
			imageStore(OUT_LIGHTING, pix, ivec4(0));
		#else
...
#else
	{
		...
		const ivec4 lightIndecesAndWeightes = imageLoad(SRC_LIGHTS_CHOSEN, pix);

		// TODO: how do this better?
	#if (ADAPT_LIGHTS_COUNT == 1)
		sampled_id[0] =		 int(lightIndecesAndWeightes.x);
		sampled_probability[0] = float(lightIndecesAndWeightes.y) / WEIGHTS_STORE_MULTIPLIER;
	#elif (ADAPT_LIGHTS_COUNT == 2)
		sampled_id[0] =		 int(lightIndecesAndWeightes.x);
		sampled_probability[0] = float(lightIndecesAndWeightes.y) / WEIGHTS_STORE_MULTIPLIER;
		sampled_id[1] =		 int(lightIndecesAndWeightes.z);
		sampled_probability[1] = float(lightIndecesAndWeightes.w) / WEIGHTS_STORE_MULTIPLIER;
	#else
		int i = 0;
...
#if LIGHT_CHOOSE_PASS
	imageStore(OUT_LIGHTING, pix, ivec4(lightWeightsIndices * vec4(1., WEIGHTS_STORE_MULTIPLIER, 1., WEIGHTS_STORE_MULTIPLIER)));
#elif OUT_SEPARATELY
