...
void R_DecalShoot( int textureIndex, int entityIndex, int modelIndex, vec3_t pos, int flags, float scale )
{
	...
	if( entityIndex > 0 )
	{
		ent = gEngine.GetEntityByIndex( entityIndex );

		if( modelIndex > 0 ) model = gEngine.pfnGetModelByIndex( modelIndex );
		else if( ent != NULL ) model = gEngine.pfnGetModelByIndex( ent->curstate.modelindex );
		else return;
	}
	else if( modelIndex > 0 )
		model = gEngine.pfnGetModelByIndex( modelIndex );
