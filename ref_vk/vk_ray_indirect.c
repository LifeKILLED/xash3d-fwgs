...
#define INIT_IMAGE(index, name, ...) \
	...
	RAY_INDIRECTIONAL_INPUTS(INIT_IMAGE)
	RAY_INDIRECTIONAL_OUTPUTS(INIT_IMAGE)
...
#define OUT(index, name, ...) -(RayResource_##name + 1),
	...
	RAY_INDIRECTIONAL_INPUTS(IN)
	RAY_INDIRECTIONAL_OUTPUTS(OUT)
#undef IN
#undef OUT
};

struct ray_pass_s *R_VkRayIndirectPassCreate( void ) {
	...
	const ray_pass_create_tracing_t rpc = {
		...
		.raygen = "ray_indirectional.rgen.spv",
		.miss = miss,
