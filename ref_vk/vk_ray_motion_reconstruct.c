...
#define LIST_COMMON_BINDINGS(X) \
	...
	X(6, all_textures, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, MAX_TEXTURES, VK_SHADER_STAGE_RAYGEN_BIT_KHR | VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR | VK_SHADER_STAGE_ANY_HIT_BIT_KHR) \
	X(7, skybox, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR) \


static const VkDescriptorSetLayoutBinding bindings_init[] = {
...
#define INIT_IMAGE(index, name, ...) \
	...
	RAY_LAST_FRAME_BUFFERS_OUTPUTS(INIT_IMAGE)

#undef INIT_IMAGE
#undef INIT_BINDING
};

static const int semantics_init[] = {
#define IN(index, name, ...) (RayResource_##name + 1),
#define OUT(index, name, ...) -(RayResource_##name + 1),
	LIST_COMMON_BINDINGS(IN)
	RAY_LAST_FRAME_BUFFERS_OUTPUTS(OUT)
#undef IN
#undef OUT
};
...
#define INIT_IMAGE(index, name, ...) \
	...
	LIST_COMMON_BINDINGS(INIT_BINDING)
	RAY_MOTION_RECONSTRUCT_INPUTS(INIT_IMAGE)
	RAY_MOTION_RECONSTRUCT_OUTPUTS(INIT_IMAGE)
...
#define OUT(index, name, ...) -(RayResource_##name + 1),
	LIST_COMMON_BINDINGS(IN)
	RAY_MOTION_RECONSTRUCT_INPUTS(IN)
	RAY_MOTION_RECONSTRUCT_OUTPUTS(OUT)
#undef IN
#undef OUT
};
...
struct ray_pass_s * R_VkRayLastFrameBuffersInitPassCreate( void ) {
	...
	const ray_pass_create_tracing_t rpc = {
		.debug_name = "motion reconstruct buffers init",
		.layout = {
			.bindings = bindings_init,
			.bindings_semantics = semantics_init,
...
struct ray_pass_s* R_VkRayMotionReconstructPassCreate(void) {
	// FIXME move this into vk_pipeline or something
	const struct SpecializationData {
		uint32_t sbt_record_size;
	} spec_data = {
	...
	const ray_pass_create_tracing_t rpc = {
		.debug_name = "motion reconstruction",
		.layout = {
			.bindings = bindings,
